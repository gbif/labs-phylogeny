...
import {  AutoComplete, Row, Col, Input, Form, message } from 'antd';
...
getData = async (value) => {
    let response = await axios.get(`//api.gbif.org/v1/species/suggest?q=${value}&datasetKey=d7dddbf4-2cf0-4f39-9b2a-bb099caae36c`);
...
getDataFromOtl = async (canonicalName, obj) => {
  ...
  try {
    ...
    const response = await axios.get(`//www.gbif.org/api/otl/ottid?canonicalName=${canonicalName}&nubKey=${nubKey}`);
    message.info("Got match in Open Tree of Life, fetching Tree")
    const treeResponse = await axios.get(`//www.gbif.org/api/otl/newick?ott_id=${_.get(response, 'data.ott_id')}`)
    this.props.setTree(_.get(treeResponse, 'data.newick'))
    this.setState({loading: false})
  } catch(err){
    console.log(err.response)
    if(_.get(err, "response.status") === 404){
      message.info(`Sorry no match in Open Tree of Life for ${canonicalName}`)
...
getTreeFromOtl = async (node_id) => {
  try {
    ...
    const treeResponse = await axios.get(`//www.gbif.org/api/otl/newick?node_id=${node_id}`)
    this.props.setTree(_.get(treeResponse, 'data.newick'))
    this.setState({loading: false})
  } catch(err){
    this.setState({loading: false})
  ...
  render() {
    ...
    <React.Fragment>
      ...
      >
          {names.map(n => <Option key={n.key} value={n.canonicalName}>{n.canonicalName}</Option>)}
