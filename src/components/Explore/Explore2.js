...
class Explore extends React.Component {
  ...
  removeNode = ({ node }) => {
    ...
    delete n[id];
        dropColor(this.state.selected[id].color);
  ...
  onToggle = ({ node, taxonKeys , firstLeaf,
    firstLeafIndex,
    lastLeaf,
    leafIndex}) => {
   // const node = this.state.nodeIdMap[selected];
    if (this.state.selected[node.id]) {
      ...
      this.setState({
        selected: {
          ...this.state.selected, [node.id]: {
            color,
            taxonKeys: taxonKeys,
            visibleTaxonKeys: taxonKeys.slice(0, 200),
            layerName: node.id,
  ...
  render() {
    ...
    return (
      <SplitPane split="vertical" minSize={200} defaultSize="50%" primary="second" style={{ overflow: 'hidden', height: this.props.hideNavigation ? '100vh' : 'calc(100vh - 68px)' }} onDragFinished={this.refreshSizes}>
         <div className="treeCard" style={{overflow: 'scroll'}}>
        <PhylogenyTree focusedNode={this.state.focusedNode} tree={this.props.newick} nameMap={this.state.nameMap} onToggle={this.onToggle} onSelect={this.onSelect} highlighted={this.state.selected}></PhylogenyTree>
        </div>
