...
class Upload extends React.Component {
  ...
  onSelect = data => {
    ...
    let taxaKeys = filtered.slice(0, catCol.length);
    console.log(taxaKeys);
    let selected = taxaKeys.map((x, i) => ({
      taxonKey: x.taxonKey,
      color: catCol[i % catCol.length],
      key: x.key
    }));
    this.setState({ taxaKeys, selected, totalSelected: filtered.length })
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div style={{ zIndex: 100, position: 'absolute', right: 30, top: 138 }}>
          <ColorBy selected={this.state.selected}></ColorBy>
        </div>
